...
These console tests install the Openshift Cluster Observability Operator with the Distributed Tracing UI Plugin in the specified cluster and then run a series of tests against the UI.
...
Set the var to install Cluster Observability, OpenTelemetry adn Tempo operators from redhat-operators catalog source.
```bash
...
```
Set the var to use customCluster Observability Operator bundle image. Tempo and OpenTelemetry operators will be installed from redhat-operators catalog source.
```bash
