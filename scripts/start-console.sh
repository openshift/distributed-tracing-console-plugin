...
PLUGIN_NAME='distributed-tracing-console-plugin'
...
if [ -x "$(command -v podman)" ]; then
    if [ "$(uname -s)" = "Linux" ]; then
        ...
        --env BRIDGE_PLUGIN_PROXY='{"services": [{"consoleAPIPath": "/api/proxy/plugin/distributed-tracing-console-plugin/backend/", "endpoint":"http://localhost:9002", "authorize": true}]}' \
        $CONSOLE_IMAGE
    else
        ...
        --env-file <(set | grep BRIDGE)  \
        --env BRIDGE_PLUGIN_PROXY='{"services": [{"consoleAPIPath": "/api/proxy/plugin/distributed-tracing-console-plugin/backend/", "endpoint":"http://host.containers.internal:9002", "authorize": true}]}' \
        $CONSOLE_IMAGE
    fi
else
    ...
    --rm -p "$CONSOLE_PORT":9000 \
    --env-file <(set | grep BRIDGE) \
    --env BRIDGE_PLUGIN_PROXY='{"services": [{"consoleAPIPath": "/api/proxy/plugin/distributed-tracing-console-plugin/backend/", "endpoint":"https://host.docker.internal:9002", "authorize": true}]}' \
    $CONSOLE_IMAGE 
fi
